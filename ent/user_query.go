...
func (uq *UserQuery) QueryHosts() *HostQuery {
...
func (uq *UserQuery) WithHosts(opts ...func(*HostQuery)) *UserQuery {
	query := (&HostClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withHosts; query != nil {
		if err := uq.loadHosts(ctx, query, nodes,
...
func (uq *UserQuery) loadHosts(ctx context.Context, query *HostQuery, nodes []*User, init func(*User), assign func(*User, *Host)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.HostsTable)
		s.Join(joinT).On(s.C(host.FieldID), joinT.C(user.HostsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.HostsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.HostsPrimaryKey[1]))

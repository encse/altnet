...
func (uc *UserCreate) SetUser(s schema.Uname) *UserCreate {
	uc.mutation.SetUser(s)
	return uc
}

// SetPassword sets the "password" field.
func (uc *UserCreate) SetPassword(sh schema.PasswordHash) *UserCreate {
	uc.mutation.SetPassword(sh)
	return uc
}
...
func (uc *UserCreate) AddHostIDs(ids ...int) *UserCreate {
	uc.mutation.AddHostIDs(ids...)
	return uc
}

// AddHosts adds the "hosts" edges to the Host entity.
func (uc *UserCreate) AddHosts(h ...*Host) *UserCreate {
	ids := make([]int, len(h))
	for i := range h {
		ids[i] = h[i].ID
	}
	return uc.AddHostIDs(ids...)
}
...
func (uc *UserCreate) check() error {
	if _, ok := uc.mutation.User(); !ok {
		return &ValidationError{Name: "user", err: errors.New(`ent: missing required field "User.user"`)}
	}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if value, ok := uc.mutation.LastLoginAttempt(); ok {
		_spec.SetField(user.FieldLastLoginAttempt, field.TypeTime, value)
		_node.LastLoginAttempt = &value
	}
	if nodes := uc.mutation.HostsIDs(); len(nodes) > 0 {
